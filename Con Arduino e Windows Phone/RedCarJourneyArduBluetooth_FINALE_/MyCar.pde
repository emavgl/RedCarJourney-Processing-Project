class MyCar
{
  ...
  public void setDestination(int posX2, int posY2)
  {
    ...
    this.posY2 = constrain(posY, 9, 660);
  }
  ...
  public void MoveMyCar(int currentVel)
  {
    int spostamento = 0;
    if (currentVel > -20 && currentVel < 20)
    { 
        BaseMovement(posX, posY);
        if (mSmoke.GetPuffIndex() < 5) mSmoke.MakeSmoke(this.posX+40, this.posY+190);
    }
    else 
    {
       ...
       if (currentVel > 10) { sinistra = true; }
       else { destra = true; }
       spostamento = currentVel/10;
       if (sinistra)
       {
         
         BaseMovement(constrain((posX -= spostamento), 62, 842), posY);
         if (mSmoke.GetPuffIndex() < 5) mSmoke.MakeSmoke(this.posX+40, this.posY+190);
       }
       if (destra)
       {
